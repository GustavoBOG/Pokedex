...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function buscarPokemon(nombrePokemon) {
    fetch(`https://pokeapi.co/api/v2/pokemon/${nombrePokemon}`)
      ...
      .then((data) => {
        currentPokemonIndex = data.id; // Actualizar el índice actual al ID del Pokémon encontrado
        ...
        const pokedexPcNoche = document.getElementById("pokedeInicioNoche");
        pokedexPcNoche.src = "./css/img/podekexPcNoche.png";

        pokemonInfo.innerHTML = ""; // Limpiar resultados anteriores y limpiar intervalos previos
        intervalos.forEach(function (intervaloId) {
          clearInterval(intervaloId); // Detener cada intervalo usando su identificador único
        });
        intervalos = []; //reseeteo de del array intervalos
        ...
        intervalos.push(
          setTimeout(
            () => mostrarDatoGradualmente("tipo", `Tipos: ${tiposEnEspanol}`),
            retraso[6]
          )
        );
      })
      ...
      .then((data) => {
        if (terminoEsNumero) {
          sugerencias = data.results.filter(
            (pokemon, index) => index + 1 === parseInt(terminoDeBusqueda)
          );
        } else {
